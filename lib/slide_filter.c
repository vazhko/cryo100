...
void filter(double *data, unsigned char ch, unsigned char do_init) {
	...
	const double coef[] = {0.01033386, 0.20756121, 0.56420986};
#else
	// двойное скольжение
	...
	f_delta = fabs(*data - ff[ch].filter[2]);
	//MOD_DOUBLE(f_delta);
	...
	*data = coef[0]*(ff[ch].filter[0] + ff[ch].filter[4]) + coef[1]*(ff[ch].filter[1] + ff[ch].filter[3]) + coef[2] * ff[ch].filter[2];
	...
	// спорный момент
	// без него это скользящее среднее
	ff[ch].filter[2] = *data;

}

/******************************************************************************************/
// функция с авто инициализацией
void filter_processor(double *data, unsigned char ch){
	
	static unsigned char init_done[FLTR_DATA_COUNT] = FLTR_DATA_INIT_VAL;
